...
- Install Bun, then run `bun install` in the command line under your repo folder.
...
- Type-checking uses the native compiler from `@typescript/native-preview` (tsgo) in `bun run build`.
- For VS Code, keep `.vscode/settings.json` with `"typescript.experimental.useTsgo": true` to enable tsgo for TS/TSX.

## Manually installing the plugin

- Copy over `main.js`, `styles.css`, `manifest.json` to your vault `VaultFolder/.obsidian/plugins/your-plugin-id/`.

## Improve code quality with Oxlint and Oxfmt

- Oxlint and Oxfmt provide fast linting and formatting with Ultracite presets.
- Run `bun run lint` (or `bun run lint:fix` to auto-fix) for linting.
