...
export class ChatView extends ItemView {
	...
	setDataHandlers(save: (data: unknown) => Promise<void>, load: () => Promise<unknown>): void {
		this.saveData = save;
	...
	async onOpen(): Promise<void> {
		...
		this.inputEl.addEventListener("input", () => {
			...
			this.inputEl.setCssProps({
				"--input-height": Math.min(this.inputEl.scrollHeight, 150) + "px",
			});
		});
	...
	private async handleSend(): Promise<void> {
		...
		if (!this.copilotService || !this.contextService || !this.settings) {
			this.showError(
				"Copilot is not initialized yet. Please wait or check the settings for connection status.",
			);
			return;
		}
	...
	private saveMessages(): void {
		...
		this.saveTimeout = setTimeout(() => {
			...
			const trimmed = this.messages.length > max ? this.messages.slice(-max) : this.messages;
