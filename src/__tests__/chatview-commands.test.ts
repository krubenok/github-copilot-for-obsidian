...
describe.serial('ChatView', () => {
  ...
  it('renders messages in the chat log', async () => {
    ...
    const appendMessage = (view as unknown as { appendMessage: (role: 'user' | 'assistant', content: string) => ChatMessage })
    ...
    const container = (view as unknown as { messagesContainer: HTMLDivElement }).messagesContainer;
  ...
  it('streams assistant responses and finalizes content', async () => {
    ...
    const appendMessage = (view as unknown as { appendMessage: (role: 'user' | 'assistant', content: string) => ChatMessage })
      .appendMessage.bind(view);
    const setStreaming = (view as unknown as { setStreaming: (streaming: boolean) => void }).setStreaming.bind(view);
    const handleStreamDelta = (view as unknown as { handleStreamDelta: (delta: string) => void }).handleStreamDelta.bind(view);
    const handleStreamComplete = (view as unknown as { handleStreamComplete: (message: ChatMessage, fullText: string) => void })
    ...
    const container = (view as unknown as { messagesContainer: HTMLDivElement }).messagesContainer;
    const contentEl = container.querySelector('.copilot-chat-message-assistant .copilot-chat-message-content') as HTMLDivElement;
    ...
    const updated = container.querySelector('.copilot-chat-message-assistant .copilot-chat-message-content') as HTMLDivElement;
  ...
  it('loads and persists message history', async () => {
    ...
    const appendMessage = (view as unknown as { appendMessage: (role: 'user' | 'assistant', content: string) => ChatMessage })
      .appendMessage.bind(view);
    const saveMessages = (view as unknown as { saveMessages: () => void }).saveMessages.bind(view);
...
describe.concurrent('registerCommands', () => {
  const buildPlugin = (options?: { activeFile?: { path: string; name: string; extension: string } | null }) => {
  ...
  it('sends selection commands into the chat view', async () => {
    ...
    expect(sendPrefilled).toHaveBeenCalledWith(expect.stringContaining('Please explain the following'));
  });
